...
router.post('/upload', authenticateToken, async (req, res) => {
  const { go_name } = req.body;

  if (!go_name) return res.status(400).json({ error: 'All details are required' });
  if (!req.files || !req.files.file) return res.status(400).json({ error: 'File is required' });
 if (hasHtmlCssJsTags(go_name)) {
    return res
      .status(400)
  ...
  try {
    uploadedFile.mv(uploadPath, async (err) => {
      ...
      const result = await pool.query(
        'INSERT INTO communities_list_go (user_id, go_name , filename, file_type) VALUES ($1, $2, $3, $4) RETURNING *',
        [req.user.userId, go_name , uploadedFile.name, uploadedFile.mimetype]
      );
...
router.get('/download/:type/:id', async (req, res) => {
  ...
  const query = `SELECT filename FROM communities_list_go WHERE id = $1 AND file_type = ANY($2::text[])`;
...
router.get('/details', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT id, go_name ,filename, file_type FROM communities_list_go');
...
router.put('/edit/:id', authenticateToken, async (req, res) => {
  ...
  const go_name = req.body.go_name; // Extract the specific field
  ...
  const checkResult = await pool.query('SELECT * FROM communities_list_go WHERE id = $1', [id]);
  ...
  try {
    ...
    if (req.files && req.files.file) {
      ...
      await pool.query(
        'UPDATE communities_list_go SET go_name = $1, filename = $2, file_type = $3 WHERE id = $4 RETURNING *',
        [go_name, uploadedFile.name, uploadedFile.mimetype, id]
      );
    } else {
      // If no file is uploaded, just update the go_name
      await pool.query(
        'UPDATE communities_list_go SET go_name = $1 WHERE id = $2 RETURNING *',
        [go_name, id]
      );
    }
...
// Delete communities_list_go
router.delete('/delete/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;

  try {
    await pool.query('DELETE FROM communities_list_go WHERE id = $1', [id]);
