...
router.post("/upload", authenticateToken, async (req, res) => {
  ...
  if (!name || !act_rules_name)
    return res.status(400).json({ error: "All details are required" });
  if (!req.files || !req.files.file)
    return res.status(400).json({ error: "File is required" });
  // Block HTML/JS/CSS injection
  if (hasHtmlCssJsTags(name) || hasHtmlCssJsTags(act_rules_name)) {
    return res
      .status(400)
  ...
  try {
    uploadedFile.mv(uploadPath, async (err) => {
      ...
      const result = await pool.query(
        "INSERT INTO actandrulestn (user_id, name,act_rules_name , filename, file_type) VALUES ($1, $2, $3, $4, $5) RETURNING *",
        [
          req.user.userId,
          name,
          act_rules_name,
...
router.get("/download/:type/:id", async (req, res) => {
  ...
  const query = `SELECT filename FROM actandrulestn WHERE id = $1 AND file_type = ANY($2::text[])`;
  const values = [
    id,
...
router.get("/details", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT id, name,act_rules_name ,filename, file_type FROM actandrulestn"
    );
...
router.put("/edit/:id", authenticateToken, async (req, res) => {
  ...
  const checkResult = await pool.query(
    "SELECT * FROM actandrulestn WHERE id = $1",
    [id]
  );
  ...
  try {
    ...
    if (req.files && req.files.file) {
      ...
      await pool.query(
        "UPDATE actandrulestn SET name = COALESCE($1, name), act_rules_name = COALESCE($2, act_rules_name),filename=COALESCE($3, filename),file_type=COALESCE($4, file_type) WHERE id = $5 RETURNING *",
...
router.delete("/delete/:id", authenticateToken, async (req, res) => {
  ...
  try {
    await pool.query("DELETE FROM actandrulestn WHERE id = $1", [id]);
