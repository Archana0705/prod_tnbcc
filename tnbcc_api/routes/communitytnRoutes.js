  ...
  try {
    uploadedFile.mv(uploadPath, async (err) => {
      ...
      const result = await pool.query(
        'INSERT INTO communities_list_tamil (user_id , filename, file_type) VALUES ($1, $2, $3) RETURNING *',
...
router.get('/download/:type/:id', async (req, res) => {
  ...
  const query = `SELECT filename FROM communities_list_tamil WHERE id = $1 AND file_type = ANY($2::text[])`;
...
router.get('/details', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT id ,filename, file_type FROM communities_list_tamil');
...
router.put('/edit/:id', authenticateToken, async (req, res) => {
  ...
  const checkResult = await pool.query('SELECT * FROM communities_list_tamil WHERE id = $1', [id]);
  ...
  try {
    ...
    if (req.files && req.files.file) {
      ...
      await pool.query(
        'UPDATE communities_list_tamil SET  filename = $1, file_type = $2 WHERE id = $3 RETURNING *',
...
// Delete communities_list_tamil
router.delete('/delete/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;

  try {
    await pool.query('DELETE FROM communities_list_tamil WHERE id = $1', [id]);
    res.status(200).json({ message: 'Community list [Tamil] deleted successfully' });
  } catch (err) {
    res.status(500).json({ error: 'Failed to delete Community list [Tamil]' });
  }
});
