...
router.post("/upload", authenticateToken, async (req, res) => {
  ...
  const budget_name = req.body.budget_name; // Extract the specific field
  ...
  if (hasHtmlCssJsTags(budget_name)) {
    return res
      .status(400)
  ...
  try {
    uploadedFile.mv(uploadPath, async (err) => {
      ...
      const result = await pool.query(
        "INSERT INTO budget_tamil (user_id, budget_name , filename, file_type) VALUES ($1, $2, $3, $4) RETURNING *",
        [req.user.userId, budget_name, uploadedFile.name, uploadedFile.mimetype]
      );
      res
        .status(201)
...
router.get("/download/:type/:id", async (req, res) => {
  ...
  const query = `SELECT filename FROM budget_tamil WHERE id = $1 AND file_type = ANY($2::text[])`;
  const values = [
    id,
...
router.get("/details", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT id, budget_name ,filename, file_type FROM budget_tamil"
    );
...
router.put("/edit/:id", authenticateToken, async (req, res) => {
  ...
  const checkResult = await pool.query(
    "SELECT * FROM budget_tamil WHERE id = $1",
    [id]
  );
  ...
  try {
    ...
    if (req.files && req.files.file) {
      ...
      await pool.query(
        "UPDATE budget_tamil SET budget_name = $1, filename = $2, file_type = $3 WHERE id = $4 RETURNING *",
        [budget_name, uploadedFile.name, uploadedFile.mimetype, id]
      );
    } else {
      // If no file is uploaded, just update the budget_name
      await pool.query(
        "UPDATE budget_tamil SET budget_name = $1 WHERE id = $2 RETURNING *",
        [budget_name, id]
      );
    }
...
router.delete("/delete/:id", authenticateToken, async (req, res) => {
  ...
  try {
    await pool.query("DELETE FROM budget_tamil WHERE id = $1", [id]);
    res.status(200).json({ message: "Budget details deleted successfully" });
  } catch (err) {
    res.status(500).json({ error: "Failed to delete Budget details" });
  }
});
