...
router.post("/upload", authenticateToken, async (req, res) => {
  ...
  // 2. XSS protection
  if (hasHtmlCssJsTags(name) || hasHtmlCssJsTags(status)) {
    return res
      .status(400)
  ...
  try {
    uploadedFile.mv(uploadPath, async (err) => {
      ...
      const result = await pool.query(
        "INSERT INTO central_obc_list (user_id, name,status , filename, file_type) VALUES ($1, $2, $3, $4, $5) RETURNING *",
        [
          req.user.userId,
          name,
          status,
...
router.get("/download/:type/:id", async (req, res) => {
  ...
  const query = `SELECT filename FROM central_obc_list WHERE id = $1 AND file_type = ANY($2::text[])`;
  const values = [
    id,
...
router.get("/details", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT id, name,status ,filename, file_type FROM central_obc_list"
    );
...
router.put("/edit/:id", authenticateToken, async (req, res) => {
  ...
  const checkResult = await pool.query(
    "SELECT * FROM central_obc_list WHERE id = $1",
    [id]
  );
  ...
  try {
    ...
    if (req.files && req.files.file) {
      ...
      await pool.query(
        "UPDATE central_obc_list SET name = COALESCE($1, name), status = COALESCE($2, status),filename=COALESCE($3, filename),file_type=COALESCE($4, file_type) WHERE id = $5 RETURNING *",
        [name, status, uploadedFile.name, uploadedFile.mimetype, id]
      );
    }
...
router.delete("/delete/:id", authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM central_obc_list WHERE id = $1",
      [id]
    );
