...
router.post("/upload", authenticateToken, async (req, res) => {
  ...
  if (hasHtmlCssJsTags(uploadedFile.name)) {
...
router.get("/details", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT id ,filename, file_type FROM community_list_english"
    );
...
router.put("/edit/:id", authenticateToken, async (req, res) => {
  ...
  const checkResult = await pool.query(
    "SELECT * FROM community_list_english WHERE id = $1",
    [id]
  );
...
router.delete("/delete/:id", authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM community_list_english WHERE id = $1",
      [id]
    );
    ...
    res
      .status(200)
      .send({ message: "Community list [English] deleted successfully" });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({ error: "Failed to delete Community list [English]" });
  }
});
